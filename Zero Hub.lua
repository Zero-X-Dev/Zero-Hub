-- nexora.xyz
if getgenv().nexora.xyz then 
	if game.CoreGui:FindFirstChild("Zero Hub GUI | True V1") then
		for i, v in ipairs(game.CoreGui:GetChildren()) do
			if string.find(v.Name,  "Zero Hub | True V1") then
				v:Destroy()
			end
		end
	end
end
getgenv().nexora.xyz = true

local DisableAnimation = game.Players.LocalPlayer.PlayerGui:FindFirstChild('TouchGui')
local T1UIColor = {
	["Border Color"] = Color3.fromRGB(220, 220, 220),              -- White/Light Gray
	["Click Effect Color"] = Color3.fromRGB(200, 200, 200),
	["Setting Icon Color"] = Color3.fromRGB(200, 200, 200),
	["Logo Image"] = "rbxassetid://140318122086207",
	["Search Icon Color"] = Color3.fromRGB(200, 200, 200),
	["Search Icon Highlight Color"] = Color3.fromRGB(255, 255, 255), -- White
	...
	["Title Text Color"] = Color3.fromRGB(240, 240, 240),          -- Off-White
	...
	["Background Image"] = "",
	["Page Selected Color"] = Color3.fromRGB(80, 80, 80),          -- Dark Gray
	["Section Text Color"] = Color3.fromRGB(200, 200, 200),
	["Section Underline Color"] = Color3.fromRGB(220, 220, 220),   -- Light Gray
	["Toggle Border Color"] = Color3.fromRGB(200, 200, 200),
	["Toggle Checked Color"] = Color3.fromRGB(255, 255, 255),      -- White
	["Toggle Desc Color"] = Color3.fromRGB(150, 150, 150),
	["Button Color"] = Color3.fromRGB(100, 100, 100),              -- Medium Gray
	["Label Color"] = Color3.fromRGB(0, 0, 0),
	["Dropdown Icon Color"] = Color3.fromRGB(200, 200, 200),
	["Dropdown Selected Color"] = Color3.fromRGB(70, 70, 70),      -- Dark Gray
	["Dropdown Selected Check Color"] = Color3.fromRGB(120, 120, 120),
	["Textbox Highlight Color"] = Color3.fromRGB(220, 220, 220),
	["Box Highlight Color"] = Color3.fromRGB(220, 220, 220),
	["Slider Line Color"] = Color3.fromRGB(200, 200, 200),
	["Slider Highlight Color"] = Color3.fromRGB(160, 160, 160),    -- Lighter Gray
...
Library_Function.Gui.Name = 'Zero Hub | True V1 GUI'
...
Library.ToggleUI = function()
	...
	if game.CoreGui:FindFirstChild("Zero Hub | True V1 GUI") then
		for a, b in ipairs(game.CoreGui:GetChildren()) do
			if b.Name == "Zero Hub | True V1 GUI" then
...
function Library:CreateWindow(Setting)
	...
	local colorB = tostring(Library_Function.Getcolor(getgenv().UIColor['Title Text Color'])[3])
	local color = colorR .. ',' .. colorG .. ',' .. colorB
    TextLabelMain.Text = tostring(TitleNameMain or "Zero Hub | True V1 - Blox Fruit")
...
task.spawn(function()
	...
	local success, err = pcall(function()
		loadstring(game:HttpGet("https://raw.githubusercontent.com/Zero-X-Dev/Zero-Hub/refs/heads/main/Zero-Hub-Fast-Attack.lua"))();
	end);
	if not success then
...
local Window = Library:CreateWindow({
    Title = "Zero Hub | True V1 - Blox Fruit",
    SubTitle = "",
...
task.delay(2, function()
	pcall(function()
		Library:Notify({
			Title = "Zero Hub | True V1",
			Content = "Script loaded successfully! Welcome., " .. game.Players.LocalPlayer.Name .. " ",
			Icon = "rocket",
			Duration = 6
		});
	end);
	Library:Notify({Title = "Zero Hub | True V1", Content = "Script loaded! Welcome, " .. game.Players.LocalPlayer.Name, Icon = "star", Duration = 5});
end);

InfoTab:AddSection(" Zero Hub | True V1 ");
InfoTab:AddParagraph({
	Title = "Zero Hub | True V1",
	Desc = "Version: R6.1 | Game: Blox Fruits\nMade for the Zero Hub Community\nAll features are free to use."
});
InfoTab:AddParagraph({
...
InfoTab:AddButton({
	...
	Callback = function()
		setclipboard("https://discord.gg/YVKbRBprdh");
		Library:Notify({Title = "Zero Hub | True V1", Content = "Discord link copied to clipboard!\ndiscord.gg/kuki-hub", Icon = "bell", Duration = 5});
	end
});
InfoTab:AddParagraph({
	Title = " Credits",
	Desc = "Dev: nexora.xyz\nOwner: nexora.xyz\n\nZero Hub | True V1 R6.1 - Blox Fruits"
});
...
-- LOOP 1: LEVEL FARM (Zero Hub | True V1)
spawn(function()
	while wait(0.2) do
	...
	while true do
		...
		pcall(function()
			...
			if _isSpecialChestItem() then
				...
				Library:Notify({Title = "Zero Hub | True V1", Content = "Item especial encontrado! Auto Chest parado.", Icon = "bell", Duration = 6});
				return;
			end;
			...
			for _, entry in ipairs(chests) do
				...
				if _isSpecialChestItem() then
					_chestTweenActive = false;
					_G.Settings.Farm["Auto Farm Chest Tween"] = false;
					Library:Notify({Title = "Zero Hub | True V1", Content = "Item especial encontrado!", Icon = "bell", Duration = 6});
					break;
				end;
			...
			if _G.ChestHopActive and _G.ChestHopCount >= _G.ChestHopLimit then
				_G.ChestHopCount = 0;
				Library:Notify({Title = "Zero Hub | True V1", Content = "Chest Hop: limite atingido, trocando server...", Icon = "bell", Duration = 4});
				task.wait(2);
				local module = (loadstring(game:HttpGet("https://raw.githubusercontent.com/raw-scriptpastebin/FE/main/Server_Hop_Settings")))();
				module:Teleport(game.PlaceId);
			end;
	...
	while true do
		...
		pcall(function()
			...
			if _isSpecialChestItem() then
				...
				Library:Notify({Title = "Zero Hub | True V1", Content = "Item especial encontrado! Chest Bypass parado.", Icon = "bell", Duration = 6});
				return;
			end;
		...
		if _G.ChestHopActive and _G.ChestHopCount >= _G.ChestHopLimit then
			_G.ChestHopCount = 0;
			Library:Notify({Title = "Zero Hub | True V1", Content = "Chest Hop: trocando server...", Icon = "bell", Duration = 4});
			task.wait(2);
			local module = (loadstring(game:HttpGet("https://raw.githubusercontent.com/raw-scriptpastebin/FE/main/Server_Hop_Settings")))();
			module:Teleport(game.PlaceId);
		end;
...
local function ShowResetConfirm()
	pcall(function()
		...
		simBtn.MouseButton1Click:Connect(function()
			...
			Library:Notify({Title = "Zero Hub | True V1", Content = "Settings resetados! Reexecute o script.", Icon = "bell", Duration = 6});
		end);
...
local function BuyFightStyleFull(styleName)
	...
	if not data then
		Library:Notify({Title = "Zero Hub | True V1", Content = "Estilo nao encontrado: " .. tostring(styleName), Icon = "alert-triangle", Duration = 4});
		return;
	end;
	local plr = game.Players.LocalPlayer;
	local hrp = plr.Character and plr.Character:FindFirstChild("HumanoidRootPart");
	...
	Library:Notify({Title = "Zero Hub | True V1", Content = "Compra enviada: " .. styleName, Icon = "check", Duration = 4});
end;
					...
					if PortalIslands and PortalIslands[_G.SelectIsland] then
						...
						if target then
							...
							if hrp and (hrp.Position - destCF.p).Magnitude < 20 then
								-- Chegou! Para o loop
								_G.BypassTeleportActive = false;
								Library:Notify({Title = "Zero Hub | True V1", Content = "Chegou em: " .. _G.SelectIsland, Icon = "map-pin", Duration = 4});
							end;
						end;
					end;
				end);
			...
			if nightTime then
				for _, v in pairs(mystic:GetDescendants()) do
					if v:IsA("MeshPart") and v.Material == Enum.Material.Neon and v.BrickColor == BrickColor.new("Bright blue") then
						...
						task.wait(0.2);
						pcall(function()
							Library:Notify({Title = "Zero Hub | True V1", Content = "Quest completa!", Icon = "bell", Duration = 6});
						end);
...
HoldAndSkillTab:AddSection("Skills Config - Select the skills to use on the farms.");

HoldAndSkillTab:AddParagraph({
	Title = "How it works",
	Desc = "Select below which keys/skills will be used in ALL farms and hub functions that require skills. The combinations Z X C Melee, Z X C V F Fruit, Z X Sword, and Z X Gun define fast groups."
});
...
HoldAndSkillTab:AddToggle({
	Title = "Z X Sword",
	Desc = "Uses only Z and X for Sword (Sword)",
	Value = false,
	Callback = function(state)
		if state then
			getgenv().HoldSkillConfig["Z"] = true;
...
HoldAndSkillTab:AddToggle({
	Title = "Usar Skill Z",
	Desc = "Enables the use of the Z key on farms",
	Value = true,
	Callback = function(state)
...
HoldAndSkillTab:AddToggle({
	Title = "Usar Skill C",
	Desc = "Enables the use of the C key on farms",
	Value = true,
	Callback = function(state)
